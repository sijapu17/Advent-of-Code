...
import string
f = open('C:/Users/Simon/SkyDrive/Home Stuff/Python/Advent of Code/2018-15.txt')
contents = f.read()
            ...
            for i in range(self.w):
                ...
                if x in ('E','G'):
                    ...
                    if x=='E':
                        ...
                        self.units[id]=Elf(self,id,pos)
                        elfI+=1
                    elif x=='G':
                        id=hiList[goblinI] #Assign next lowercase letter to elf
                        self.units[id]=Goblin(self,id,pos)
                        goblinI+=1
                else:
                    self.map[pos]=x
...
class Unit(): #Parent class for a unit, which will be either an Elf or a Goblin
    ...
    def __init__(self,cave,id,pos):
        self.cave=cave
        self.id=id
        self.pos=pos
        self.hp=200
        self.power=3 #Attack power
        self.alive=True
...
def solveA(input): #Find round where combat ends
    cave=Cave(input)
    print(cave)
    while not cave.endOfBattle:
        nRounds=cave.runRound()
        print('Round '+str(cave.rounds))
        print(cave)
    return(nRounds)
...
##gobs=e0.getEnemies()
##elves=g0.getEnemies()
##d0=g0.getDestinations(elves)
##d1=g1.getDestinations(elves)
##p0=g0.getStep(d0)
##p1=g1.getStep(d1)