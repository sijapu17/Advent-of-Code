...
class Maze(): #Overall class for maze containing map, keys and doors
                ...
                if x=='.': #Check whether path has adjacent portal 
                    for d in dirs:
                        ...
                        if input[int(adj.imag)][int(adj.real)] in string.ascii_uppercase:
                            ...
                            if min(pos.real,pos.imag)<=3 or min(self.w-pos.real,self.h-pos.imag)<=3:
                                pDict=self.outerPortals
                            else:
                                pDict=self.innerPortals
                                label=label.lower() #lowercase indicates inner portal
                                #Use first inner portal to calculate size of donut
                                if self.donutSize==None:
                                    self.donutSize=int(pos.imag)-3
    ...
    def astar(self): #A* search to find shortest path for collecting all keys
        ...
        node=AStarNode(self,0,'AA',0,[('AA',0)]) #Start node at entrance 'AA'
        heapq.heappush(openHeap,node)
        n=0
        while len(openHeap)>0 and n<100000:
            ...
            for dest in self.paths[start]: #Loop through all possible portals that can be reached from current portal
                ...
                if dest=='ZZ':
                    if level==0:
                        ...
                        continue #'ZZ' not valid outside level 0
                else:
                    if dest.isupper(): #Jump out a level
                        if level==0:
                            continue #Cannot jump outwards from level 0
                        childLevel=level-1
                        childDest=dest.lower()
                    else: #Jump in a level
                        childLevel=level+1
                        childDest=dest.upper()
                    childDist=node.dist+path.dist+1 #Add 1 to account for jumping through portal
                if (childDest,childLevel) in node.path:                
                    continue #Do not go through a portal more than once in the same level                
                childPath=node.path.copy()
                childPath.append((dest,childLevel))
                child=AStarNode(self,childLevel,childDest,childDist,childPath)
                if n==102:
...
class AStarNode(): #Node for A*
    ...
    def __init__(self,maze,level,current,dist,path):
        ...
        self.f=self.dist+self.maze.donutSize*self.level
    ...
    def __str__(self):
        return('f='+str(self.f)+' lv='+str(self.level)+' dist='+str(self.dist)+str(self.path))
