...
f = open('C:/Users/Simon/SkyDrive/Home Stuff/Python/Advent of Code/2019/2019-21.txt')
contents = f.read()
...
class System(): #Springdroid system
    ...
    def runDroid(self,script): #Run springdroid
        for instr in script:
...
def solve(code,script):
    droid=System(code)
    return(droid.runDroid(script))

scriptA=('NOT C J','AND D J','NOT A T','OR T J','WALK')

retA=solve(code,scriptA)

scriptB=('NOT C J','AND D J','AND H J','NOT B T','AND D T','OR T J','NOT A T','OR T J','RUN')

retB=solve(code,scriptB)
